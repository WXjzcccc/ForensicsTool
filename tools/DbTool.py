...
class DbTool:
    ...
    def decrypt_EnMicroMsg(self, key: str, path: str):
        """
        @key:       数据库密钥
        @path:      需要解密的数据库路径
        """
        ...
        result = subprocess.run([sqlite3, path,
                                 f"PRAGMA key = '{key}';PRAGMA cipher_compatibility = 1;ATTACH DATABASE 'EnMicroMsg_dec.db' AS EnMicroMsg_dec KEY '';SELECT sqlcipher_export('EnMicroMsg_dec');DETACH DATABASE EnMicroMsg_dec;"],
        ...
        if out.strip() == 'ok' and err == '':
            ...
            ret = "EnMicroMsg.db解密成功，解密后数据库在源文件同目录下EnMicroMsg_dec.db"
    ...
    def decrypt_FTS5IndexMicroMsg(self, key: str, path: str):
        """
        @key:       数据库密钥
        @path:      需要解密的数据库路径
        """
        ...
        result = subprocess.run([sqlite3, path,
                                 f"PRAGMA key = '{key}';PRAGMA kdf_iter = 64000;PRAGMA cipher_kdf_algorithm = PBKDF2_HMAC_SHA1;PRAGMA cipher_hmac_algorithm = HMAC_SHA1;ATTACH DATABASE 'FTS5IndexMicroMsg.db' AS FTS5IndexMicroMsg KEY '';SELECT sqlcipher_export('FTS5IndexMicroMsg');DETACH DATABASE FTS5IndexMicroMsg;"],
                                capture_output=True, text=True)
        out = result.stdout
        err = result.stderr
        os.chdir(work_dir)
        if out.strip() == 'ok' and err == '':
            print_green(
                '[成功]---->FTS5IndexMicroMsg_encrypt.db解密成功，解密后数据库在源文件同目录下FTS5IndexMicroMsg.db')
            ret = "FTS5IndexMicroMsg_encrypt.db解密成功，解密后数据库在源文件同目录下FTS5IndexMicroMsg.db"
            return ret, 1
        else:
            print_red('[失败]---->FTS5IndexMicroMsg_encrypt.db解密失败')
            if err.strip().__contains__('already exists'):
                print_red(f'[失败]---->原因[数据库已解密过，解密后的文件已存在！]')
                return "数据库已解密过，解密后的文件已存在！", -1
            elif err.strip().__contains__('file is not a database'):
                print_red(f'[失败]---->原因[解密密钥或解密参数不正确！]')
                return "解密密钥或解密参数不正确！", -1
            else:
                print_red(f'[失败]---->原因[{err.strip()}]')
                return err.strip(), -1
    ...
    def decrypt_SQLCipher4_default(self, key: str, path: str):
        """
        @key:       数据库密钥
        @path:      需要解密的数据库路径
        """
        ...
        result = subprocess.run([sqlite3, path,
                                 f"PRAGMA key = '{key}';ATTACH DATABASE '{name}_dec.db' AS {name}_dec KEY '';SELECT sqlcipher_export('{name}_dec');DETACH DATABASE {name}_dec;"],
                                capture_output=True, text=True)
        out = result.stdout
        err = result.stderr
        os.chdir(work_dir)
        if out.strip() == 'ok' and err == '':
            ...
            return f'{bname}解密成功，解密后数据库在源文件同目录下{name}_dec.db', 1
        else:
    ...
    def decrypt_wcdb_default(self, key: str, path: str):
        """
        @key:       数据库密钥
        @path:      需要解密的数据库路径
        """
        ...
        print_yellow_key(f'[提示]---->正在解密数据库{bname}，解密密钥为', key)
        result = subprocess.run([sqlite3, path,
                                 f"PRAGMA key = '{key}';PRAGMA cipher_page_size = 4096;PRAGMA kdf_iter = 64000;PRAGMA cipher_kdf_algorithm = PBKDF2_HMAC_SHA1;PRAGMA cipher_hmac_algorithm = HMAC_SHA1;ATTACH DATABASE '{name}_dec.db' AS {name}_dec KEY '';SELECT sqlcipher_export('{name}_dec');DETACH DATABASE {name}_dec;"],
                                capture_output=True, text=True)
        out = result.stdout
        err = result.stderr
        os.chdir(work_dir)
    ...
    def decrypt_SQLCipher3_default(self, key: str, path: str):
        """
        @key:       数据库密钥
        @path:      需要解密的数据库路径
        """
        ...
        print_yellow_key(f'[提示]---->正在解密数据库{bname}，解密密钥为', key)
        result = subprocess.run([sqlite3, path,
                                 f"PRAGMA key = '{key}';PRAGMA cipher_page_size = 1024;PRAGMA kdf_iter = 64000;PRAGMA cipher_kdf_algorithm = PBKDF2_HMAC_SHA1;PRAGMA cipher_hmac_algorithm = HMAC_SHA1;ATTACH DATABASE '{name}_dec.db' AS {name}_dec KEY '';SELECT sqlcipher_export('{name}_dec');DETACH DATABASE {name}_dec;"],
                                capture_output=True, text=True)
        out = result.stdout
        err = result.stderr
        os.chdir(work_dir)
    ...
    def decrypt_amap(self, file: str):
        ...
        with open(file, 'rb') as fr:
            ...
            if err != '':
                ...
                return "girf_sync.db解密失败，未知原因，可能是更换了密钥！", -1
            else:
                print_green('[成功]---->girf_sync.db解密成功，解密后数据库在源文件同目录下girf_sync_dec.db')
                return "girf_sync.db解密成功，解密后数据库在源文件同目录下girf_sync_dec.db", 1
        ...
        for v in cpu:
            ...
            with open(path, 'rb') as fr:
                ...
                if head == b'SQLite format 3\0':
                    print_green(
                        f'[成功]---->{bname}解密成功，解密后数据库在源文件同目录下{bname.replace(".db", "_dec.db")}')
                    return f'{bname}解密成功，解密后数据库在源文件同目录下{bname.replace(".db", "_dec.db")}', 1
                else:
                    print_red(f'[失败]---->{bname}解密失败，解密密钥不正确！')
        print_red(f'[失败]---->{bname}解密失败，未能找到密钥，请确认输入的内容！')
        return f'{bname}解密失败，未能找到密钥，请确认输入的内容！', -1

    def decrypt_ntqq(self, uid: str, path: str):
        """
        @uid:       用户的UID
        @path:      需要解密的数据库路径
        """
        ...
        for hmac in hmac_lst:
            result = subprocess.run([sqlite3, path,
                                     f"PRAGMA key = '{key}';PRAGMA cipher_page_size = 4096;PRAGMA kdf_iter = 4000;PRAGMA cipher_kdf_algorithm = PBKDF2_HMAC_SHA512;PRAGMA cipher_hmac_algorithm = {hmac};ATTACH DATABASE '{name}_dec.db' AS {name}_dec KEY '';SELECT sqlcipher_export('{name}_dec');DETACH DATABASE {name}_dec;"],
                                    capture_output=True, text=True)
            out = result.stdout
            err = result.stderr
            if out.strip() == 'ok' and err == '':
