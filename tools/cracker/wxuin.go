...
func crackUin(target []string, start int, end int, c chan string, ctx context.Context, wg *sync.WaitGroup) {
	defer wg.Done()
	for i := start; i <= end; i++ {
	...
	*/
	...
	for i := -2147483648; i <= 2147483647; i += 100000000 {
		...
		if i+99999999 < 2147483647 {
			go crackUin(target, i, i+99999999, c, f.crackCtx, &wg)
		} else {
			go crackUin(target, i, 2147483647, c, f.crackCtx, &wg)
		}
		Counts++
	}
	wg.Wait()
